# КОМАНДЫ ДЛЯ ИСПРАВЛЕНИЯ НА ПРОДАКШН СЕРВЕРЕ

## 1. Создание диагностического скрипта

```bash
cat > fix_tracking_status.py << 'EOF'
#!/usr/bin/env python3
"""
...
```bash
python3 fix_tracking_status.py
```

## 3. Исправление в web/app.py

Найдите в файле `web/app.py` функцию `api_status()` (примерно строка 170-180):
...
```bash
curl -s "https://cleverdriver.ru/api/status" | python3 -c "import sys, json; data=json.load(sys.stdin); print('API response:', data)"
```

## Ожидаемый результат после исправления

Все 3 теста должны пройти:
- ✅ Проверка полей API: ПРОЙДЕН
- ✅ Тест стабильности статуса: ПРОЙДЕН  
- ✅ Тест консистентности страницы: ПРОЙДЕН
...
2. Убедитесь, что файл изменен:
   ```bash
   grep -A5 -B5 "tracking.*tracking_active" web/app.py
   ``` 