...
async def main():
    ...
    try:
        ...
        # Отключаем прокси для избежания ошибок SOCKS
        if hasattr(application.bot, '_request') and hasattr(application.bot._request, '_session'):
            application.bot._request._session.proxies = {}
            application.bot._request._session.trust_env = False
